...
function validateAmount(amount: number): boolean {
  return amount > 0 && amount % 10 === 0 && amount <= 100000000 // 최대 1억원
}

function sanitizeString(input: string): string {
  return input.trim().replace(/[<>\"']/g, '') // XSS 방지
}
...
export async function addSpending(spending: Omit<Spending, "id" | "createdAt">): Promise<Spending> {
  try {
    ...
    if (!spending.category || spending.category.trim().length === 0) {
      throw new Error("카테고리를 선택해주세요")
    }
    if (!spending.userName || spending.userName.trim().length === 0) {
      throw new Error("사용자 이름이 필요합니다")
    }
    if (spending.userName.length > 20) {
      throw new Error("사용자 이름은 20자 이하여야 합니다")
    }
    if (spending.memo && spending.memo.length > 200) {
    ...
    const insertData = {
      ...
      description: sanitizeString(spending.memo || ""),
