...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validateCategory(category, INCOME_CATEGORIES)) {
    ...
    console.log("[v0] 수입 저장 요청:", sanitizeLogData({ amount, category, userName, familyCode, clientIP }))
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id || !validateId(id)) {
      ...
      return NextResponse.json(
        { success: false, error: "유효하지 않은 수입 ID입니다" },
        { status: 400 },
      )
    }

    console.log("[v0] 수입 삭제 요청:", sanitizeLogData({ id, clientIP }))
